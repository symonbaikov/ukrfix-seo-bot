...
def select_task() -> tuple[str, str, str] | None:
    """
    Select a random combination of country, city, category that hasn't been posted yet.
    This uses a deterministic pool of remaining combinations instead of random attempts.
    """
    ...
    posted = set(get_posted_combinations())
    remaining = [combo for combo in all_combinations if combo not in posted]
...
def main() -> None:
    ...
    while True:
        try:
            ...
            if not task:
                log(
                    "All combinations posted! Resetting posted list and starting over..."
                )
                reset_posted()
                time.sleep(5)
                continue
            ...
            article = generate_article(
                country, city, category, google_context, recent_titles
            )
            ...
            article.html_content = inject_internal_links(
                article.html_content, internal_links
            )
